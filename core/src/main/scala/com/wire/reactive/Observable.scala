...
trait Observable[Listener] {
  ...
  @volatile private[reactive] var wired = false
  @volatile private var listeners = Set.empty[Listener]

  protected def onWire(): Unit
  protected def onUnwire(): Unit

  private[reactive] def subscribe(l: Listener): Unit = listenersMonitor.synchronized {
    listeners += l
    if (!wired) {
      wired = true
      onWire()
  ...
  private[reactive] def unsubscribe(l: Listener): Unit = listenersMonitor.synchronized {
    listeners -= l
  ...
  private[reactive] def notifyListeners(invoke: Listener => Unit): Unit = listeners foreach invoke

  private[reactive] def hasSubscribers = listeners.nonEmpty
